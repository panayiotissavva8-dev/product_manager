...
# --- Ensure DB exists ---
RUN mkdir -p build && \
    sqlite3 $DB_PATH "CREATE TABLE IF NOT EXISTS users(user_id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password_hash TEXT, role TEXT);" && \
...
# --- Start the app ---
CMD ["bash", "-c", "export PORT=${PORT:-18080} && ./build/product_manager_app"]